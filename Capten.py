...
cl = LINETCR.LINE()
#cl.login(qr=True)
cl.login(token="EoWK9ngQ3K5dvtEo3L8d.N+2++oUtuxydwiH9p8nOpq.w3XY7vbh76lDesMisqd3xv9+RzpdLAd9BafpOjVQzpU=")
cl.loginResult()
print "Cl-Login Success\n"

ki = LINETCR.LINE()
#ki.login(qr=True)
ki.login(token="EoeSqydNZyu04FCltwa0.UMdscWFyOgm0EwIf6eHmWa.XLrc94rCBzefD47MdVKH4WIYAg3o5Y6/B+PvzXtmhEI=")
ki.loginResult()
print "Ki-Login Success\n"

kk = LINETCR.LINE()
#kk.login(qr=True)
kk.login(token="Eoj17zRkWVWgI185kQg1.jCYvLUpT3Vo7eoA0Ji8cCq.vVtgH5hmsuGoprRCSYfoYZsQgD4LYMY4iRiOXexTvio=")
kk.loginResult()
print "Kk-Login Success\n"

kc = LINETCR.LINE()
#kc.login(qr=True)
kc.login(token="Eot27XfinA65Iy914K6a.kGmGKr+Qmm2Mj/7juVptoG.b9eLFbwWZMvjcVvW0+Fxt6iFXrZdpZwrvcuLgUTKHLw=")
kc.loginResult()
print "Kc-Login Success\n"

kr = LINETCR.LINE()
#kr.login(qr=True)
kr.login(token="EoilFoIWRHP3PYLBFGM8.bJVOlcXxauHdg04pUtpHAa.6VHnOG5QdWpvYFgtQVIT8ihlxWlusYrsXAaKMAeym3I=")
kr.loginResult()
print "Kr-Login Success\n"

km = LINETCR.LINE()
#km.login(qr=True)
km.login(token="Eom0dBiB2h7JvAof7YL9.tGuwjeoLpMwCvmSwsRf6oq.u4fUNRHCUMVcpIl+uLPQzRDjAyBDKYmTqgwHa9PyyJI=")
km.loginResult()
...
Creator=["uaff92c78fa04ded8a225fe8e55c83edb","u334247c1497182bbf12323959743ef4d"]
admin=["uaff92c78fa04ded8a225fe8e55c83edb'","u334247c1497182bbf12323959743ef4d"]
...
#		    cl.sendText(msg.to,"")
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cnkapten","Kapten")
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cntc1","TC1")
                    if len(string.decode('utf-8')) <= 5000:
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cntc2","TC2")
                    if len(string.decode('utf-8')) <= 5000:
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cntc3","TC3")
                    if len(string.decode('utf-8')) <= 5000:
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cntc4","TC4")
                    if len(string.decode('utf-8')) <= 5000:
                        profile = cl.getProfile()
                        profile.displayName = string
                        kr.updateProfile(profile)
...
#              if msg.from_ in admin:
                ...
                cl.sendText(msg.to,"Tapi masih tamvan juga")
                ki.sendText(msg.to,"Tak tun tuang")
                kk.sendText(msg.to,"Tak tun tuang")
                cl.sendText(msg.to,"apalagi kalau sudah mandi")
                ki.sendText(msg.to,"Tak tun tuang")
                kk.sendText(msg.to,"Pasti tamvan sekali")
